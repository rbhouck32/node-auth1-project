...
server.use(
  session({
    // huge
    name: "monkey", // the default would be session id, but that would reveal our stack
    ...
    resave: false, // we might need to set this to true to avoid idle sessions being deleted (google this.)
    saveUninitialized: false, // keep it false to avoid storing sessions and sending cookies for unmodified sessions

    // lets persist sessions in the db so sessions don't die on server restarts
